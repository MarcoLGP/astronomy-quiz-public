    ...
    return (
        <>
            ...
            <Row align="center" justify="space-between" className={styles.box_end_footer_container}>
                ...
                <button onClick={() => push(result?.result ? `/home/challenge/${challengeNumber + 1}/start` : `/home/challenge/${challengeNumber}/start`)} className={styles.box_end_footer_next_challenge_btn}>{result?.result ? "Próximo desafio" : "Tentar novamente"}</button>
            </Row>
        </>
    );
